...
from utlis.config import MEMORY_FILEPATH, SIMILARITY_THRESHOLD, MAX_MEMORY_PER_EXPERT
...
class LLMMemory:
    def __init__(self, model_name:str, generation_config:dict, safety_settings:dict):
        ...
        try:
            self.model = GeminiService # Use passed GeminiService instant for modularity
    ...
    async def query(self, user_id:str, expert_name:str, question:str):
        """
            Query the GeminiService while considering user context.
        """
        try: 
...
async def run_model_with_memory(user_id:str, expert_name:str, prompt:str, llm:GeminiService) -> str:
    """
